...
export default function MyBlogsClient() {
  ...
  const { myBlogs, setBlogToEdit, setMyBlogs, removeFromMyBlogs } =
    useBlogStore();
  ...
  useEffect(() => {
    ...
    setMyBlogs(blogs || []);
  }, [userBlogsResponse?.data?.blogs, setMyBlogs]);
  ...
  const userBlogsTotalPages = Math.ceil(
    (myBlogs.length || 0) / userBlogsPageSize,
  );
  ...
  const userBlogsPagination = {
    ...
    paginationData: {
      totalElements: myBlogs.length || 0,
  ...
  const paginatedUserBlogs = useMemo(() => {
    return (myBlogs || []).slice(userBlogsStartIndex, userBlogsEndIndex);
  }, [myBlogs, userBlogsStartIndex, userBlogsEndIndex]);
  ...
  const handleDeleteConfirm = async () => {
    ...
    removeFromMyBlogs(blogToDelete);
    ...
    <div className="min-h-screen flex items-center justify-center py-12 relative">
      <div className="container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-8">
          <BlogsPageHeader isMyBlogs={true} onCreateNew={handleCreate} />
          ...
          <UserBlogsSection
            ...
            isLoading={isLoading}
            showPagination={(myBlogs.length || 0) > 12}
